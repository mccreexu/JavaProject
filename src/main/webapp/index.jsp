		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="empUpdateForm" class="form-horizontal">
	...
	<script type="text/javascript">
		...
		function validate_add_form(){
			...
			if(!regName.test(empName)){
				//alert("用户名为2-5位中文或6-16位英文和数字的组合");
		...
		$("#emp_save_btn").click(function(){
			...
			//0.5、判断之前的ajax请求校验用户名是否成功，成功了才 保存
		...
		$("#check_all").click(function(){
			//dom原生的属性用prop读取或修改；attr用于获取或修改自定义属性的值
			//让每一个员工前面的复选框属性的值（即选中/未选中）跟全选键一致
			$(".check_item").prop("checked",$(this).prop("checked"));
		});
		
		//每个员工的复选框都被选中时，全选键也应该被选中
		$(document).on("click",".check_item",function(){
			//判断被选中的复选框个数是否等于当前页面元素个数
		...
		$("#emp_delete_all_btn").click(function(){
			...
			//方法名是substring，不是subString
			empNames = empNames.substring(0,empNames.length-2);
			//弹出确认删除提示框
			if(confirm("是否删除所有被选中的员工：【" + empNames + "】")){
				$.ajax({
